...
class UsersFactory {
    ...
    func byContributions(pyro:Pyro) -> UsersViewModel {
        var viewModel:UsersViewModel = UsersViewModel()
        for user:UserProtocol in self.usersByContributions(pyro:pyro) {
            var item:UsersItem = UsersItem()
            item.name = self.makeName(user:user)
            item.user = user
    ...
    func byStreak(pyro:Pyro) -> UsersViewModel {
        var viewModel:UsersViewModel = UsersViewModel()
        for user:UserProtocol in self.usersByStreak(pyro:pyro) {
            var item:UsersItem = UsersItem()
            item.name = self.makeName(user:user)
            item.user = user
            if let metrics:Metrics = user.metrics {
    ...
    private func usersByContributions(pyro:Pyro) -> [UserProtocol] {
        return pyro.users.sorted { (userA:UserProtocol, userB:UserProtocol) -> Bool in
    ...
    private func usersByStreak(pyro:Pyro) -> [UserProtocol] {
        return pyro.users.sorted { (userA:UserProtocol, userB:UserProtocol) -> Bool in
            if let metricsA:Metrics = userA.metrics {
                if let metricsB:Metrics = userB.metrics {
